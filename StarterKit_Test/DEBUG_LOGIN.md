...
## Problem
The login endpoint at `api/StarterKit/SecureAuth/v1/login` is hanging - requests are sent but no response is received.
...
Look for:
- `POST /api/StarterKit/SecureAuth/v1/login` endpoint
...
```
Server=.;Database=StarterKit;MultipleActiveResultSets=False;TrustServerCertificate=True;Trusted_Connection=True;Pooling=true;Max Pool Size=100;Connection Timeout=30;
```

**Check:**
- SQL Server is running
- Database `StarterKit` exists
...
**Endpoint URL:**
```
POST http://localhost:5000/api/StarterKit/SecureAuth/v1/login
```
...
- Check if database exists: `SELECT name FROM sys.databases WHERE name = 'StarterKit'`
